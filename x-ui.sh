...
install() {
    bash <(curl -Ls https://raw.githubusercontents.com/lg-yyds/xui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    ...
    bash <(curl -L -s https://raw.githubusercontents.com/lg-yyds/xui/main/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/lg-yyds/xui/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue() {
    wget -N https://raw.githubusercontents.com/lg-yyds/xui/main/acme1key.sh && bash acme1key.sh
}
